...
public class CreateGameChannels extends BothelperSubcommandData {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        if (categoryChannelName != null && !categoryChannelName.isEmpty()) {
            List<Category> categoriesWithName = AsyncBot.jda.getCategoriesByName(categoryChannelName, false);
            if (categoriesWithName.size() > 1) {
                ...
                categoryChannel = AsyncBot.jda.getCategoriesByName(categoryChannelName, false).get(0);
            }
    ...
    private static boolean gameOrRoleAlreadyExists(String name) {
        List<Guild> guilds = AsyncBot.jda.getGuilds();
    ...
    private static Guild getNextAvailableServer() {
        if (serverCanHostNewGame(AsyncBot.guildPrimary)) {
            return AsyncBot.guildPrimary;
        } else if (serverCanHostNewGame(AsyncBot.guildSecondary)) {
            return AsyncBot.guildSecondary;
        } else {
            return null;
        }
    }
    ...
    public static List<Category> getAllAvailablePBDCategories() {
        ...
        return AsyncBot.jda.getCategories().stream()
